param(
    [string]$SmokeRoot = (Join-Path $env:TEMP "te-component-inventory-sync-smoke")
)
...
$env:TE_SYNC_SMOKE_ROOT = $resolvedSmokeRoot

Write-Host "TE Lab Components Inventory one-machine sync smoke"
...
try {
    ...
    Remove-Item Env:\TE_SYNC_SMOKE_ROOT -ErrorAction SilentlyContinue
}
