...
const OLD_DB_STEMS: &[&str] = &[
    "te_component_inventory",
    "te_lab_components_inventory",
    "te_lab_components_shared",
];
...
mod tests {
    ...
    fn unique_backup_path_keeps_existing_backup_files() {
        let root = env::temp_dir().join(format!(
            "te-component-inventory-db-quarantine-{}",
            Uuid::new_v4().simple()
        ));
    ...
    fn move_file_falls_back_to_copy_remove_semantics() {
        let root = env::temp_dir().join(format!(
            "te-component-inventory-db-move-{}",
            Uuid::new_v4().simple()
        ));
        fs::create_dir_all(&root).unwrap();
