...
const TEST_DB_PATH = "D:/coding/TE Lab Components Inventory/app-data/inventory.feox";
...
describe("InventoryShell entry actions", () => {
  ...
  it("opens the editor on row double click and updates the entry", async () => {
    ...
    await user.dblClick(screen.getByText("GH series 4-position crimp housings"));
  ...
  it("archives an entry from the right-click menu", async () => {
    ...
    fireEvent.contextMenu(screen.getByText("0.1 uF ceramic capacitors, 0603"));
    expect(screen.queryByText("Entry Actions")).not.toBeInTheDocument();
    expect(screen.getAllByText("0.1 uF ceramic capacitors, 0603")).toHaveLength(1);
    await user.click(await screen.findByRole("button", { name: "Archive Entry" }));

    expect(screen.queryByText("0.1 uF ceramic capacitors, 0603")).not.toBeInTheDocument();
    ...
    expect(await screen.findByText("0.1 uF ceramic capacitors, 0603")).toBeInTheDocument();
  });
  ...
  it("restores an entry from the right-click menu without a browser prompt", async () => {
    ...
    await user.click(screen.getAllByRole("button", { name: /Archive/i })[0]);
    fireEvent.contextMenu(screen.getByText("Legacy microcontroller boards"));
    await user.click(await screen.findByRole("button", { name: "Restore Entry" }));

    expect(screen.queryByText("Legacy microcontroller boards")).not.toBeInTheDocument();
  ...
  it("opens a styled delete dialog and cancels without deleting", async () => {
    ...
    expect(screen.getAllByText("0.1 uF ceramic capacitors, 0603").length).toBeGreaterThan(0);
    expect(confirm).not.toHaveBeenCalled();

    await user.click(screen.getByRole("button", { name: "Cancel" }));

    expect(screen.queryByText("Delete this entry?")).not.toBeInTheDocument();
    expect(screen.getByText("0.1 uF ceramic capacitors, 0603")).toBeInTheDocument();
  });
  ...
  it("hides the saved-link action when a row has no link", () => {
    ...
    fireEvent.contextMenu(screen.getByText("10 kOhm precision resistors, 0603"));
  ...
  it("does not open unsafe saved-link schemes", async () => {
    ...
    window.inventoryDesktop = {
      ...
      exportExcel: vi.fn().mockResolvedValue({ canceled: false, outputPath: "D:/exports/TE_Component_Inventory_Export.xlsx" }),
    };
