...
describe("InventoryShell views and exports", () => {
  ...
  it("switches to archive view and updates the summary", async () => {
    ...
    expect(screen.getByText("Legacy microcontroller boards")).toBeInTheDocument();
  });
  ...
  it("shows and clears the filter panel", async () => {
    ...
    await user.type(manufacturerFilter, "Murata");
  ...
  it("updates theme preference and shows mock verified feedback", async () => {
    ...
    expect(localStorage.getItem("teComponentInventory.theme")).toBe("dark");

    await user.click(screen.getByRole("button", { name: /Toggle verified for GH series 4-position crimp housings/i }));
  ...
  it("runs desktop Excel export when available", async () => {
    ...
    const exportExcel = vi.fn().mockResolvedValue({
      canceled: false,
      outputPath: "D:/exports/TE_Component_Inventory_Export.xlsx",
    });
