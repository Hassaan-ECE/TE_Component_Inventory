...
describe("InventoryShell loading and search", () => {
  ...
  it("renders the inventory view by default with seeded counts", () => {
    ...
    expect(screen.getAllByText("TE Lab Components Inventory")).toHaveLength(1);
  ...
  it("does not show bundled mock data when desktop database loading fails", async () => {
    ...
    expect(screen.queryByText("GH series 4-position crimp housings")).not.toBeInTheDocument();
    expect(screen.getByText("Could not load the TE Lab Components Inventory database.")).toBeInTheDocument();
  });
  ...
  it("filters desktop search locally without querying the backend per keystroke", async () => {
    ...
    const desktopEntries = [
      ...
      buildTestEntry({ id: "802", description: "Digital caliper", manufacturer: "Murata" }),
    ];
    ...
    expect(await screen.findByText("Showing all 2 entries")).toBeInTheDocument();
    await user.type(screen.getByLabelText("Inventory search"), "mur");

    await waitFor(() => expect(screen.getByText('1 result for "mur"')).toBeInTheDocument());
