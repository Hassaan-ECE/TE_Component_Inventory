...
describe("inventory helpers", () => {
  ...
  it("combines field filters with global search", () => {
    const results = filterEntries(MOCK_INVENTORY, "inventory", "controller", {
      ...DEFAULT_FILTERS,
      location: "cabinet b",
    });

    expect(results).toHaveLength(1);
    expect(results[0]?.id).toBe("te-1004");
  });
  ...
  it("formats long links into compact table labels", () => {
    expect(
      formatLinkLabel(
        "https://www.jst-mfg.com/product/index.php?series=105&tracking=demo",
      ),
    ).toBe("www.jst-mfg.com/product/index.php");
  });
